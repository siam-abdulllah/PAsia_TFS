app.controller("ExpRequisitionRecommendCtrl", function ($scope, $http, uiGridConstants) {
    $scope.EventPerm(44);
    ...
    $scope.SaveData = function () {
        var expReqPrepareDtlData = $scope.gridExpReqRecommendDtlOptions.data;
        ...
        $scope.SaveDb.RecommendedRemarks = $scope.RecommendedRemarks;
        $scope.SaveDb.RecommendedStatus = $scope.RecommendedStatus;
        ...
        $scope.SaveDb.TotalApprovedAmt = $scope.gridExpReqRecommendDtlApi.grid.columns[6].getAggregationValue();
        $http({
            method: "POST",
            url: MyApp.rootPath + "ExpRequisitionRecommend/" + methodName,
            data: { expReqPrepareMstInfo: $scope.SaveDb, expReqPrepareDtlData: expReqPrepareDtlData }
        }).then(function (response) {
            if (response.data.Status === "Ok") {
                methodName = "UpdateExpReqRecommendedInfo";
    ...
    $scope.GetExpReqMstList = function () {
        ...
        $http({
            method: "POST",
            url: MyApp.rootPath + "ExpRequisitionRecommend/GetExpReqMstList",
            data: { param: param }
        }).then(function (response) {
            if (response.data.Status === "Ok") {
                if (response.data.Data.length > 0) {
                    $scope.gridExpReqRecommendMstOptions.data = response.data.Data;
                    $('#ExpReqRecommendMstModal').modal('show');
                } else {
    ...
    $scope.GetExpRecommendedReqMstList = function () {
        var param = "";
        $http({
            method: "POST",
            url: MyApp.rootPath + "ExpRequisitionRecommend/GetExpRecommendedReqMstList",
            data: { param: param }
        }).then(function (response) {
            //if (response.data.Status === "Ok") {
            if (response.data.length > 0) {
                $('#ExpReqRecommendMstModal').modal('show');
                $scope.gridExpReqRecommendMstOptions.data = response.data;
            } else {
                toastr.warning("No Data Found!", { timeOut: 2000 });
            }
            //} else {
    ...
    $scope.GetExpReqDtlList = function () {
        ...
        $http({
            method: "POST",
            url: MyApp.rootPath + "ExpRequisitionRecommend/GetExpReqDtlList",
            data: { mstId: $scope.Id }
        }).then(function (response) {
            if (response.data.Status === "Ok") {
                if (response.data.Data.length > 0) {
                    $scope.gridExpReqRecommendDtlOptions.data = response.data.Data;
                }
            } else {
                toastr.warning(response.data.Status, { timeOut: 2000 });
            }
    ...
    var columnExpReqRecommendMstInfo = [
        ...
        { name: 'PrepareName', displayName: "PreparedName", visible: false },
        { name: 'PrepareDesig', displayName: "PreparedDesig", visible: false },
        { name: 'PrepareRemarks', displayName: "PreparedRemarks", visible: false },
    ...
    $scope.gridExpReqRecommendMstOptions = {
        showColumnFooter: true,
        enableFiltering: true,
        enableSorting: true,
        columnDefs: columnExpReqRecommendMstInfo,
    ...
    $scope.rowDblClickMst = function (row) {
        ...
        if (row.entity.RecommendedStatus !== "Pending") {
            $scope.RecommendedDate = row.entity.RecommendedDate;
            $scope.RecommendedStatus = row.entity.RecommendedStatus;
            $scope.RecommendedRemarks = row.entity.RecommendedRemarks;
            methodName = "UpdateExpReqRecommendedInfo";
        } else {
            $scope.RecommendedStatus = "";
            methodName = "InsertExpReqRecommendedInfo";
        }
        if (row.entity.ApprovedStatus === "Approved" || row.entity.ApprovedStatus === "Not Approved") {
            $scope.gridExpReqRecommendDtlOptions.columnDefs[12].visible = false;
            $scope.isSaveDisable = true;
        }
        else {
            $scope.gridExpReqRecommendDtlOptions.columnDefs[12].visible = true;
            $scope.isSaveDisable = false;
        }
        $scope.GetExpReqDtlList();
        $('#ExpReqRecommendMstModal').modal('hide');
    };
    var columnExpReqRecommendDtlInfo = [
        ...
        { name: 'Remarks', displayName: "Remarks" },
        {
            ...
            cellTemplate: '<div style="text-align:center;position: relative;width:100%;padding:2px 2px 2px 6px;"><button  class="btn-success" ng-click="grid.appScope.editExpReqRecommendDtlOptionsRow(row)"><i class="fa fa-edit"></i>&nbspEdit</button></div>'
        }
    ];
    $scope.gridExpReqRecommendDtlOptions = {
        showColumnFooter: true,
        enableFiltering: true,
        enableSorting: true,
        columnDefs: columnExpReqRecommendDtlInfo,
        enableGridMenu: false,
        enableSelectAll: true,
        onRegisterApi: function (gridApi) {
            $scope.gridExpReqRecommendDtlApi = gridApi;
        }
    };
    $scope.showPopUpExpReqRecommendDtlOptions = function () {
        $scope.DtlId = "";
        $scope.Mop = "";
        $scope.Purpose = "";
        $scope.PrepareValue = "";
        ...
        $scope.TotalDays = "";
        index = null;
        $('#ExpReqRecommendDtlModal').modal('show');
    };
    $scope.editExpReqRecommendDtlOptionsRow = function (row) {
        ...
        index = $scope.gridExpReqRecommendDtlOptions.data.indexOf(row.entity);
        $('#ExpReqRecommendDtlModal').modal('show');
    };

    $scope.addItemGridExpReqRecommendDtlOptions = function () {
        if (index !== null) {
        ...
        $scope.Remarks = "";
        index = null;
        $('#ExpReqRecommendDtlModal').modal('hide');
    };
    //----------- Reset ----------------//
    ...
    $scope.Reset = function () {
        ...
        $scope.PaymentPlace = "";
        $scope.gridExpReqRecommendDtlOptions.data = [];
        $scope.gridExpReqRecommendDtlOptions.columnDefs[12].visible = true;
        $scope.btnSaveValue = "Save";
        $scope.isSaveDisable = false;
