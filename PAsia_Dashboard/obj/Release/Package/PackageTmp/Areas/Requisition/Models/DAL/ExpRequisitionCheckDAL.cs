...
{
    public class ExpRequisitionCheckDAL : ReturnData
    {
        ...
        internal object GetExpCheckedReqMstList(string param)
        {
            try
            {
                string deptCode = System.Web.HttpContext.Current.Session["DEPARTMENT_CODE"].ToString();
                ...
                string qry = "SELECT  DISTINCT MST_ID,  REQUISITION_NO,  REQUISITION_TYPE,  EXPENDITURE_MONTH, " +
                    ...
                    " NVL( CHECKED_STATUS,'Pending') CHECKED_STATUS,CHECKED_REMARKS,NVL( VERIFIED_STATUS,'Pending') VERIFIED_STATUS,NVL( RECOMMENDED_STATUS,'Pending') RECOMMENDED_STATUS," +
                    " NVL( APPROVED_STATUS,'Pending') APPROVED_STATUS, CHECKED_DATE, VERIFIED_DATE," +
            ...
            try
            {
                ...
                var item = (from DataRow row in dt.Rows
                            select new ExpReqPrepareDtl
                            {
                                ...
                                CheckedValue = Convert.ToInt32(row["CHECKED_VALUE"]) == 0 ? Convert.ToInt32(row["PREPARED_VALUE"]) : Convert.ToInt32(row["CHECKED_VALUE"]),
        ...
        //-------------------- Insert ----------------------------//
        internal bool InsertExpReqCheckInfo(ExpReqPrepareMst expReqPrepareMstInfo, List<ExpReqPrepareDtl> expReqPrepareDtlData)
        {
                ...
                using (OracleCommand cmd = new OracleCommand())
                {
                    ...
                    try
                    {
                        ...
                        string qryMst =
                           ...
                           " CHECKED_STATUS=:CheckedStatus,TOTAL_APPROVED_VALUE=:TotalApproveAmt WHERE  MST_ID=:MaxID";
                        ...
                        if (noOfRowsMst > 0)
                        {
                            foreach (var data in expReqPrepareDtlData)
                            {
                                ...
                                string qryDtl =
                                    "UPDATE  EXP_REQUISITION_DTL SET  CHECKED_VALUE=:CheckedValue WHERE DTL_ID=:DtlId";
                                OracleParameter[] paramDtl = new OracleParameter[]
                                {
                                    new OracleParameter("DtlId", data.DtlId),
                                    new OracleParameter("CheckedValue","0")
                                };
                                if (expReqPrepareMstInfo.CheckedStatus == "Approved")
                                {
                                    paramDtl = new OracleParameter[]
                                   {
                                    new OracleParameter("DtlId", data.DtlId),
                                    new OracleParameter("CheckedValue", data.CheckedValue)
                                   };
                                }
                            ...
                            if (expReqPrepareMstInfo.CheckedStatus == "Approved")
                            {
                                ...
                                _auditTrailDAO.InsertAudit("frmExpRequisitionCheck", "EXP_REQUISITION_MST", IUMode, MaxID, mailBody, mailSub);
                                return isTrue;
                            }
                            _auditTrailDAO.InsertAudit("frmExpRequisitionCheck", "EXP_REQUISITION_MST", IUMode, MaxID);
                            return isTrue;
                        }
                    }
                    catch (Exception e)
        ...
        internal bool UpdateExpReqCheckInfo(ExpReqPrepareMst expReqPrepareMstInfo, List<ExpReqPrepareDtl> expReqPrepareDtlData)
        {
            if (expReqPrepareMstInfo.PreparedByConfirm != "Yes")
            {
                ExceptionReturn = "Requistion No: " + expReqPrepareMstInfo.RequisitionNo + " is not submitted yet";
            ...
            using (OracleConnection con = new OracleConnection(dbConnection.SAConnStrReader("Sales")))
            {
                using (OracleCommand cmd = new OracleCommand())
                {
                    ...
                    try
                    {
                        ...
                        IUMode = "U";
                        string qryMst =
                         "UPDATE  EXP_REQUISITION_MST SET  CHECKED_REMARKS=:CheckedRemarks, CHECKED_STATUS=:CheckedStatus,TOTAL_APPROVED_VALUE=:TotalApproveAmt WHERE  MST_ID=:MaxID";
