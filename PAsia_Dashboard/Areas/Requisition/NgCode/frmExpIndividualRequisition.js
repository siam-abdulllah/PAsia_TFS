app.controller("ExpIndividualRequisitionCtrl", function ($scope, $http, uiGridConstants) {
    $scope.EventPerm(49);
    ...
    $scope.GetExpApprovedReqMstList = function () {
        ...
        $http({
            method: "POST",
            url: MyApp.rootPath + "ExpIndividualRequisition/GetExpAllReqMstList",
            data: { param: param }
    ...
    $scope.GetExpReqDtlList = function () {
        $http({
            method: "POST",
            url: MyApp.rootPath + "ExpIndividualRequisition/GetExpReqDtlList",
            data: { mstId: $scope.Id }
        }).then(function (response) {
            if (response.data.Status === "Ok") {
                if (response.data.Data.length > 0) {
