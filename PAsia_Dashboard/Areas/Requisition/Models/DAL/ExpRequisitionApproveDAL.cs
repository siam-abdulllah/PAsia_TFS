...
namespace PAsia_Dashboard.Areas.Requisition.Models.DAL
{
    public class ExpRequisitionApproveDAL : ReturnData
    {
        ...
        internal object GetExpReqMstList(string param)
        {
            try
            {
                ...
                string qry = "SELECT DISTINCT MST_ID, REQUISITION_NO,  REQUISITION_TYPE,  EXPENDITURE_MONTH, " +
                    ...
                    " AND VERIFIED_STATUS='Approved' AND RECOMMENDED_STATUS='Approved' AND  APPROVED_STATUS IS NULL " + param + " " +
                    //" ORDER BY  REQUISITION_NO,TO_DATE(RECOMMENDED_DATE,'dd/MM/YYYY') DESC";
        ...
        internal object GetExpReqDtlList(string mstId)
        {
            try
            {
                string qry = "SELECT DTL_ID, MST_ID, MOP,PREPARED_VALUE,CHECKED_VALUE,VERIFIED_VALUE,RECOMMENDED_VALUE,NVL(APPROVED_VALUE,0) APPROVED_VALUE,PURPOSE, " +
